...
{p 4 8}{cmd:precision(}{it:string}{cmd:)} specifies the storage type for numeric variables generated by {cmd:scplotmulti} for plotting. Options are {opt double} (default) and {opt single}. {opt precision(single)} uses Stata's default generated numeric storage type, matching earlier package behavior.{p_end}
